...
export function EmptyState({
  ...
  return (
    <section className={cn('rounded-3xl border border-dashed border-[var(--editable-border)] bg-white p-10 text-center', className)}>
      <div className={cn('mx-auto flex h-14 w-14 items-center justify-center rounded-full', ui.tintStrong)}>
        <SearchX className="h-6 w-6 text-[#315fe8]" />
      </div>
      <h2 className="mt-5 text-2xl font-black tracking-tight">{title}</h2>
      <p className={cn('mx-auto mt-3 max-w-xl', ui.body)}>{description}</p>
      <Link href={actionHref} className={cn('mt-6', ui.btnOutlineAccent)}>
        {actionLabel}
